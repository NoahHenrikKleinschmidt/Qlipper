...
def main():
    ...
    if data_input:
        ...
        slider = st.slider("Threshold", min_value = float(tmin), max_value = float(tmax), value = float(2 * tmin), step=float((tmax-tmin)/500))   
        ...
        for k in keys:
            ...
            # if no intersection is found, we set simply to largest possible index 
            if idx.size == 0: idx = len(xs)-2
            else: idx = int(idx[0])
